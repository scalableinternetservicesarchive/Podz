...
class ItemCreationTest < ActionDispatch::IntegrationTest
  ...
  test "Item created by user should be associated with that user" do
    item = Item.new(title: "New Item", description: "This is a new item", category_id: 1, price_hourly_usd: "20", price_daily_usd: "100", user_id: @user.id, condition: "Good", latitude: 10, longitude: 10)
    log_in_as(@user)
    ...
    created_item = Item.find_by(title: item.title, description: item.description, category_id: item.category_id, price_hourly_usd: item.price_hourly_usd, price_daily_usd: item.price_daily_usd, user_id: @user.id)
